...
const StatCard = ({ icon: Icon, label, value, color, trend }) => {
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: -5 }}
            className="glass-card p-6 flex flex-col gap-4 border-white/5 hover:border-white/10 transition-all cursor-default group"
        >
            <div className="flex justify-between items-start">
                ...
                {trend && (
                    <span className={`text-xs font-medium px-2 py-1 rounded-lg ${trend > 0 ? 'text-green-400 bg-green-400/10' : 'text-red-400 bg-red-400/10'
                        }`}>
                        {trend > 0 ? '+' : ''}{trend}%
                    </span>
                )}
